...
public:
    ...
    ZedSensorRvizNode() : Node("zed_sensor_rviz") {
        ...
        custom_pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>("zed/custom_pose", 10);
        custom_path_pub_ = create_publisher<nav_msgs::msg::Path>("zed/custom_path", 10);
        ...
        custom_path_msg_.header.frame_id = "odom";
...
private:
    void tick() {
        ...
        if (has_imu_) {
            ...
            // 3. Custom IMU Odometry Integration with Calibration & ZUPT
            if (!is_calibrated_) {
                // Collect calibration samples to estimate gyroscope bias and world gravity vector
                if (calib_samples_ < target_calib_samples_) {
                    // Set custom_q_ to align with the current ZED camera orientation
                    custom_q_ = Eigen::Quaternionf(q.ow, q.ox, q.oy, q.oz);
                    
                    Eigen::Vector3f acc_local_eigen(filtered_acc_local_.x, filtered_acc_local_.y, filtered_acc_local_.z);
                    Eigen::Vector3f acc_world_eigen = custom_q_ * acc_local_eigen;

                    gyro_bias_ += Eigen::Vector3f(filtered_gyro_local_.x, filtered_gyro_local_.y, filtered_gyro_local_.z);
                    accel_gravity_ += acc_world_eigen;
                    
                    calib_samples_++;
                    
                    // Keep position and velocity locked to the initial visual tracker pose during calibration
                    custom_position_ = t;
                    custom_velocity_ = sl::Translation(0.0f, 0.0f, 0.0f);
                    prev_acc_local_ = filtered_acc_local_;
                    first_acc_stored_ = true;
                    last_time_ = now;
                } else {
                    // Compute final calibration averages
                    gyro_bias_ /= static_cast<float>(target_calib_samples_);
                    accel_gravity_ /= static_cast<float>(target_calib_samples_);
                    
                    RCLCPP_INFO(this->get_logger(), "IMU Calibration Complete. World Gravity vector: (%.3f, %.3f, %.3f) m/s^2, Gyro biases: (%.3f, %.3f, %.3f) deg/s",
                                accel_gravity_.x(), accel_gravity_.y(), accel_gravity_.z(),
                ...
                // Clamp dt to handle potential system delays or timing anomalies safely
                if (dt > 0.0 && dt < 0.2) {
                    // 1. Correct Gyroscope reading with calibrated bias
                    Eigen::Vector3f raw_gyro(filtered_gyro_local_.x, filtered_gyro_local_.y, filtered_gyro_local_.z);
                    Eigen::Vector3f corrected_gyro = raw_gyro - gyro_bias_;

                    // Convert angular velocity to radians/second
                    float wx = corrected_gyro.x() * M_PI / 180.0f;
                    float wy = corrected_gyro.y() * M_PI / 180.0f;
                    float wz = corrected_gyro.z() * M_PI / 180.0f;

                    // Update orientation quaternion using exponential map integration
                    float theta_x = wx * dt;
                    float theta_y = wy * dt;
                    float theta_z = wz * dt;
                    float angle = std::sqrt(theta_x * theta_x + theta_y * theta_y + theta_z * theta_z);
                    Eigen::Quaternionf dq;
                    if (angle > 1e-6f) {
                        float sin_half = std::sin(angle * 0.5f);
                        dq = Eigen::Quaternionf(std::cos(angle * 0.5f),
                                                (theta_x / angle) * sin_half,
                                                (theta_y / angle) * sin_half,
                                                (theta_z / angle) * sin_half);
                    } else {
                        dq = Eigen::Quaternionf(1.0f, 0.5f * theta_x, 0.5f * theta_y, 0.5f * theta_z);
                    }
                    custom_q_ = (custom_q_ * dq).normalized();

                    // 2. Rotate local filtered acceleration to custom world frame using custom_q_
                    Eigen::Vector3f acc_local_eigen(filtered_acc_local_.x, filtered_acc_local_.y, filtered_acc_local_.z);
                    Eigen::Vector3f acc_world_eigen = custom_q_ * acc_local_eigen;

                    // 3. Remove calibrated gravity vector (in world frame)
                    Eigen::Vector3f linear_acc = acc_world_eigen - accel_gravity_;

                    // 4. Zero Velocity Update (ZUPT) / Static Detection
                    float acc_change = 0.0f;
                    if (first_acc_stored_) {
                        acc_change = std::sqrt(
                            std::pow(filtered_acc_local_.x - prev_acc_local_.x, 2) +
                            std::pow(filtered_acc_local_.y - prev_acc_local_.y, 2) +
                            std::pow(filtered_acc_local_.z - prev_acc_local_.z, 2)
                        );
                    }
                    prev_acc_local_ = filtered_acc_local_;
                    first_acc_stored_ = true;

                    float gyro_mag = corrected_gyro.norm(); // magnitude in deg/s

                    // If the motion is below these thresholds, assume stationary to prevent drift.
                    // Gyroscope magnitude and relative acceleration change are extremely robust static indicators.
                    // Thresholds are optimized for hand-held tremors and table vibrations.
                    bool is_static = (gyro_mag < 3.0f) && (acc_change < 0.08f);

                    if (is_static) {
                        custom_velocity_.x = 0.0f;
                        custom_velocity_.y = 0.0f;
                        custom_velocity_.z = 0.0f;
                        linear_acc.setZero();
                        
                        // Slowly track gravity in the world frame to absorb orientation drift
                        accel_gravity_ = 0.95f * accel_gravity_ + 0.05f * acc_world_eigen;
                    } else {
                        // Apply a deadband threshold to filter out small sensor noise while moving
                        if (std::abs(linear_acc.x()) < 0.05f) linear_acc.x() = 0.0f;
                        if (std::abs(linear_acc.y()) < 0.05f) linear_acc.y() = 0.0f;
                        if (std::abs(linear_acc.z()) < 0.05f) linear_acc.z() = 0.0f;
                    }

                    // 5. Update Position ( p = p + v*dt + 0.5*a*dt^2 )
                    custom_position_.x += custom_velocity_.x * dt + 0.5f * linear_acc.x() * (dt * dt);
                    custom_position_.y += custom_velocity_.y * dt + 0.5f * linear_acc.y() * (dt * dt);
                    custom_position_.z += custom_velocity_.z * dt + 0.5f * linear_acc.z() * (dt * dt);

                    // 6. Update Velocity ( v = v + a*dt ) - NO damping during active motion to preserve physics!
                    custom_velocity_.x += linear_acc.x() * dt;
                    custom_velocity_.y += linear_acc.y() * dt;
                    custom_velocity_.z += linear_acc.z() * dt;
                }
                last_time_ = now;
            }
            ...
            custom_pose_msg.header.stamp = now;
            custom_pose_msg.header.frame_id = "odom";
            custom_pose_msg.pose.position.x = custom_position_.x;
            custom_pose_msg.pose.position.y = custom_position_.y;
            custom_pose_msg.pose.position.z = custom_position_.z;
            custom_pose_msg.pose.orientation.x = custom_q_.x();
            custom_pose_msg.pose.orientation.y = custom_q_.y();
            custom_pose_msg.pose.orientation.z = custom_q_.z();
            custom_pose_msg.pose.orientation.w = custom_q_.w();
            custom_pose_pub_->publish(custom_pose_msg);

            // Update and Publish Custom Path
            custom_path_msg_.header.stamp = now;
            custom_path_msg_.poses.push_back(custom_pose_msg);
            if (custom_path_msg_.poses.size() > 2000) {
                custom_path_msg_.poses.erase(custom_path_msg_.poses.begin());
            }
            custom_path_pub_->publish(custom_path_msg_);
    ...
    rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr custom_pose_pub_;
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr custom_path_pub_;
    rclcpp::TimerBase::SharedPtr timer_;

    sl::Camera zed_;
    bool has_imu_ = false;
    nav_msgs::msg::Path path_msg_;
    nav_msgs::msg::Path custom_path_msg_;

    // Custom IMU Odometry State Variables
    rclcpp::Time last_time_;
    bool is_first_tick_ = true;
    Eigen::Quaternionf custom_q_;
    sl::Translation custom_velocity_{0.0f, 0.0f, 0.0f};
    sl::Translation custom_position_{0.0f, 0.0f, 0.0f};

    // Calibration and drift correction variables
    bool is_calibrated_ = false;
    int calib_samples_ = 0;
    const int target_calib_samples_ = 90; // 3 seconds at 30Hz
    Eigen::Vector3f gyro_bias_{0.0f, 0.0f, 0.0f};
    Eigen::Vector3f accel_gravity_{0.0f, 0.0f, 0.0f};
    sl::Translation prev_acc_local_{0.0f, 0.0f, 0.0f};
    bool first_acc_stored_ = false;
